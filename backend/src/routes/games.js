...
// POST /api/games
// body: {
//   players: [{ userId: int, deckId: int }],  // 3-5 entries
//   winnerId: int,
//   winnerDeckId: int,
//   notes?: string
// }
router.post('/', auth, async (req, res) => {
  ...
  if (!players || players.length < 3 || players.length > 5)
    return res.status(400).json({ error: 'Servono 3-5 giocatori' });
  if (!normalizedWinnerId || !normalizedWinnerDeckId)
    return res.status(400).json({ error: 'Vincitore richiesto' });
  ...
  if (normalizedPlayers.some((player) => !player.userId || !player.deckId)) {
    return res.status(400).json({ error: 'Ogni giocatore deve avere utente e mazzo validi' });
  }

  const uniqueUsers = new Set(normalizedPlayers.map((player) => player.userId));
  if (uniqueUsers.size !== normalizedPlayers.length) {
    return res.status(400).json({ error: 'Lo stesso giocatore non può essere al tavolo due volte' });
  }

  // Verifica che il vincitore sia nel tavolo
  const winnerInGame = normalizedPlayers.some(
    (player) => player.userId === normalizedWinnerId && player.deckId === normalizedWinnerDeckId
  );
  if (!winnerInGame)
    return res.status(400).json({ error: 'Il vincitore deve essere nel tavolo' });
  ...
  if (invalidDeckOwnership) {
    return res.status(400).json({ error: 'Ogni deck deve appartenere al giocatore selezionato' });
  }
...
// DELETE /api/games/:id  (solo admin o chi ha registrato)
router.delete('/:id', auth, async (req, res) => {
  ...
  if (!game.createdByUserId || game.createdByUserId !== req.user.id) {
    return res.status(403).json({ error: 'Solo chi ha registrato la partita può eliminarla' });
  }
